...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (req.method === 'GET' && req.url === '/dogs') {
      // Your code here
      let dogsHTML = fs.readFileSync('views/dogs.html', 'utf-8');

      const resBody = dogsHTML
      .replace(/#{dogsList}/g, dogs.map(item => `<li>${item.name}</li>`).join(""))
    ...
    if (req.method === 'GET' && req.url === '/dogs/new') {
      // Your code here
      let createDogHTML = fs.readFileSync('views/create-dog.html', 'utf-8');
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        console.log(dog)
        let dogDetailsHTML = fs.readFileSync('views/dog-details.html', 'utf-8');

        const resBody = dogDetailsHTML
    ...
    if (req.method === 'POST' && req.url === '/dogs') {
      ...
      dogs.push({dogId: newId, ...req.body});
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 4 && urlParts[3] === 'edit') {
        ...
        let editDogHTML = fs.readFileSync('views/edit-dog.html', 'utf-8');

        const resBody = editDogHTML
        .replace(/#{name}/g, dog.name)
        .replace(/#{age}/g, dog.age)
